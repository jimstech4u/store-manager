-- 0150 — The live tracking link stays about the order, never about the customer
--
-- 0149 put the customer's whole account on the live tracking link: everything they owe and every
-- container they hold, as at the sale. The receipt should say that, and now does. The TRACKING LINK
-- must not, and this puts it back exactly as it was before 0149.
--
-- The tracking page can be opened by a five-character order code — short enough to read across a
-- counter, and so short enough to guess — and `public_track_order` answers a code with the order's
-- share token. Anything `public_track_token` returns is therefore readable by anyone who guesses a
-- code. Its own page says so: "nothing behind it may be worth guessing for … nothing about the
-- customer at all". A customer's balance and their crates are exactly what is worth guessing for.
--
-- The customer's account travels on the RECEIPT link (`read_shared_receipt`), whose token is issued
-- deliberately by the shop to that customer and cannot be derived from anything printed or spoken.
-- The helpers `customer_containers_as_at` and `customer_owed_as_at` stay: the till's receipt and the
-- receipt link use them.
--
-- Restored from the definition live immediately before 0149, byte for byte.
