-- 0096 — A receipt knows what it is holding
--
-- A deposit taken on a sale is money the shop has and must give back. `record_sale` writes it onto
-- the deposit ledger as a rate, which is how `customer_deposits_held` totals what a customer has
-- with the shop — and that part works.
--
-- But settling asks a narrower question: how much is held against THIS RECEIPT. It reads
-- `deposit_holdings`, and nothing has ever written a row there for a sale. `hold_receipt_deposit`
-- was added in 0076 for precisely this and has no caller — not in SQL, not in the app.
--
-- So the settle screen reads "Nothing was held for these" on a receipt that took ₦500, and the shop
-- can neither apply that money to a shortfall nor hand it back. It took money it had no way to
-- return, which is the worst shape a gap can have.
--
-- Found by the benchmark, which took a deposit and then asked where it went.
--
-- ONLY WITH A CUSTOMER. A walk-in has no account to hold anything against, and `record_sale`
-- already refuses to let a walk-in leave with returnables unless the deposit is taken as cash.

-- ─── And a word for releasing one ───────────────────────────────────────────────────
--
-- `deposit_holdings.reason` is a closed set — 'taken', 'refunded', 'applied_to_shortfall' — which is
-- right: this table exists to explain money, and a free-text reason explains nothing a month later.
--
-- Voiding a sale releases a holding and is none of those three. 0094 writes 'sale_voided' and would
-- have been refused by the constraint the first time a voided sale actually had a deposit on it —
-- which no test had reached, because until this migration a sale never wrote a holding at all. Two
-- gaps hiding each other.

alter table public.deposit_holdings drop constraint if exists deposit_holdings_reason_check;
alter table public.deposit_holdings add constraint deposit_holdings_reason_check
  check (reason in ('taken', 'refunded', 'applied_to_shortfall', 'sale_voided'));
